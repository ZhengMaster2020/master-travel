...
+ 第一种方式：通过`import`局部注册组件方式，在使用Header/Footer组件的那个组件注册头部尾部组件

例如：我在文件夹`views/Home.vue`，使用Header和Footer这两个组件，具体如下：
...
+ 另一种方式就是`全局注册组件`方式，不用在每个要使用到Header以及Footer时的组件，都要去import进来再去注册组件这么麻烦。具体实现方式如下：
  ...
  + 在其他组件中去直接使用导出的这些公用组件。这里我在`App.vue`中去使用Header以及Footer组件
